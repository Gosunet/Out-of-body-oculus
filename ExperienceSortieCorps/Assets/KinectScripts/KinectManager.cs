...
public class KinectManager : MonoBehaviour 
{
	...
	// Public Bool to determine whether to show hint about detected sensor height and angle. A user must stay in front of the sensor.
	...
	// Public Bool to determine whether to allow wrist and hand rotations
	public bool allowHandRotations = false;

	// Lists of AvatarController-objects that will be controlled by Kinect users
	public List<AvatarController> _avatarControllers;
    public List<AvatarController> avatarControllers
    {
        get
        {
            if (_avatarControllers.Count == 0)
            {
                ...
                foreach (MonoBehaviour monoScript in monoScripts)
                {
                    if (typeof(AvatarController).IsAssignableFrom(monoScript.GetType()))
                    {
                        ...
                        _avatarControllers.Add(avatar);
                    }
                }
            }
            return _avatarControllers;
        }

        set
        {
            if(_avatarControllers != value)
            {
                _avatarControllers = value;
            }
        }
    }
...
//				dwFlags |= KinectInterop.FrameSource.TypeAudio;
			...
			sensorData.hintHeightAngle = hintHeightAngle;
		...
		if(computeUserMap)
		{
			...
			usersLblTex = new Texture2D(sensorData.depthImageWidth, sensorData.depthImageHeight, TextureFormat.RGBA32, false);
		...
		for(int i = 0; i < sensorData.bodyCount; i++)
		{
			   ...
			   (maxUserDistance <= 0f || Mathf.Abs(bodyData.position.z) <= maxUserDistance))
			{
				   ...
				   bodyData.joint[(int)KinectInterop.JointType.ShoulderRight].trackingState != KinectInterop.TrackingState.NotTracked)
				{
					...
					Quaternion turnRot = FromToRotationFixed(Vector3.right, shouldersDir);
		...
		for(int j = 0; j < jointCount; j++)
		{
			...
			if(bJointValid)
			{
				...
				if(nextJoint != joint && nextJoint >= 0 && nextJoint < sensorData.jointCount)
				{
					...
					if(bNextJointValid)
					{
						jointOrientNormal = FromToRotationFixed(baseDir, jointDir);
					}
					   ...
					   (joint == (int)KinectInterop.JointType.Neck))
					{
						...
						Vector3 jointDir2 = bodyData.shouldersDirection;
						jointDir2.z = -jointDir2.z;
						
						jointData.normalRotation *= FromToRotationFixed(baseDir2, jointDir2);
					}
					...
					{
						Vector3 baseDir2 = Vector3.right;
						Vector3 jointDir2 = bodyData.hipsDirection;
						jointDir2.z = -jointDir2.z;
