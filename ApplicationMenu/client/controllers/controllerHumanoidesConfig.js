menu.controller('HumanoidesConfigCtrl', function ($scope, $state, $http, $rootScope, $localStorage) {
    ...
    initHumanoide = function () {
        ...
        $scope.largeur = Number(window.localStorage["local_hum_largeur"]);
    };
	...
	var sendMessage = function () {
        ...
        if($state.current.name == "humanoides")
			...
			type2 = "ba";
		var message = type + '/' + $scope.nbRepet + '_' + $scope.nbIntervalle + '_' + $scope.intervalleMin + '_' + $scope.intervalleMax + '_' + type2 + '_' + $scope.largeur;
		$http.get(message);
    ...
    saveAllValues = function (){
        ...
        window.localStorage["local_hum_largeur"] = $scope.largeur;
    };

    // Watch function to update result value and save all values
    $scope.$watch('nbRepet + nbIntervalle + intervalleMin + intervalleMax + largeur', function () {
